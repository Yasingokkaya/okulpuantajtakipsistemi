...
async function saveModalChanges() {
    ...
    if (!puantajData[personelId]) {
        puantajData[personelId] = {};
    }
    ...
    puantajData[personelId].dailyData[date] = {
        ...
        hours: newHours,
        baseHours: newHours,
        gorevlendirmeHours: 0,
        notes: newNotes
    };
    updateTableCell(personelId, date);

    try {
        const statusRow = document.querySelector(`.status-cell[data-personel-id="${personelId}"]`)?.closest('tr');
        if (statusRow) {
            const newMonthlyHours = await calculateMonthlyHours(personelId);
            const newStatusCounts = calculateStatusCounts(personelId);
            const personelData = puantajData[personelId];
            const sozlesmeTuru = personelData.sozlesme_turu;
            let sgkPrimGunu = 0;
            const periodStartDate = new Date(personelData.effectiveStartDate + 'T00:00:00Z');
            const periodEndDate = new Date(personelData.effectiveEndDate + 'T00:00:00Z');
            const gunFarki = (periodEndDate - periodStartDate) / (1000 * 60 * 60 * 24) + 1;
            sgkPrimGunu = gunFarki - ((newStatusCounts.R || 0) + (newStatusCounts.Üİ || 0));

            let nColumnContent = newStatusCounts.N || 0;
            if (sozlesmeTuru === 'Kısmi Süreli') {
                sgkPrimGunu = Math.round(newMonthlyHours.genelToplam / 7.5);
                nColumnContent = sgkPrimGunu;
            }
            statusRow.querySelector('[data-summary="aylikToplam"]').textContent = newMonthlyHours.genelToplam;
            ...
            statusRow.querySelector('[data-code="İ"]').textContent = newStatusCounts.İ || 0;
            statusRow.querySelector('[data-code="R"]').textContent = newStatusCounts.R || 0;
            statusRow.querySelector('[data-code="Üİ"]').textContent = newStatusCounts.Üİ || 0;
            statusRow.querySelector('[data-code="S"]').textContent = newStatusCounts.S || 0;
            statusRow.querySelector('[data-code="K"]').textContent = newStatusCounts.K || 0;
            statusRow.querySelector('[data-code="RTÇ"]').textContent = newStatusCounts.RTÇ || 0;
            statusRow.querySelector('[data-summary="sgkPrim"]').textContent = sgkPrimGunu;
        }
    } catch (error) {
        console.error("Satır toplamları güncellenirken hata:", error);
        showToast('Toplamlar güncellenirken bir hata oluştu.', 'error');
    }
                ...
                if (isResmiTatil(dateUTC)) {
                    ...
                    if (isWeekend) {
                        ...
                        if (personel.unvan && (personel.unvan.includes('Okul Müdürü') || personel.unvan.includes('Rehber Öğretmen') || personel.unvan.includes('Rehberlik'))) {
                            ...
                            hours = (personel.sozlesme_turu === 'Belirsiz Süreli') ? 7.5 : (scheduleData[personel.id]?.[dayOfWeekTR]?.total || 0);
                        }
                    }
                }
            }
...
async function renderPuantajTable(nobetVerisi, aktifPersonelListesi, baslikMetni, calculatedHoursMap) {
    ...
    for (const grupAdi of grupSiralama) {
        ...
        for (const personel of personelGrubu) {
            ...
            // --- YENİ KONTROL: Geçmişte ayrılan personeli gizle ---
            if (personel.isten_ayrilis && personel.isten_ayrilis !== '---' && personel.isten_ayrilis.trim() !== '') {
                const ayrilisTarihi = new Date(personel.isten_ayrilis);
                const donemBasi = new Date(year, month, 1); // Seçilen ayın 1'i
                
                // Eğer ayrılış tarihi, seçilen ayın başından küçükse (yani önceki aylarda ayrılmışsa)
                if (ayrilisTarihi < donemBasi) {
                    continue; // Bu personeli listeye koyma, bir sonrakine geç
                }
            }
            // -----------------------------------------------------
            ...
            // DÜZELTME: 'return' yerine 'continue' kullanıldı. 
            // return kullanırsan bir kişinin verisi eksikse tüm tablo durur. continue sadece o kişiyi atlar.
            if (!personel.dailyData) { continue; } 
            ...
            tableHTML += `<td class="summary-yellow" rowspan="2" data-summary="aylikToplam">${monthlyHours.genelToplam}</td>${(grupAdi === 'Belirsiz Süreli') ? `<td class="summary-overtime" rowspan="2" data-summary="fazlaMesai">${fazlaMesaiSaat}</td>${mesaiNedeniInput}${eksikGunKoduSelect}` : `<td class="summary-green clickable-ekders" rowspan="2" data-summary="ekDers"
...
async function addNobet() {
    ...
    try {
        ...
        if (!querySnapshot.empty) {
            ...
            if (personelDoc.exists()) {
                existingPersonelName = personelDoc.data().ad_soyad;
            }
        ...
        console.error("Nöbet eklenirken/güncellenirken hata: ", error);
        showToast('Nöbet işlemi sırasında bir hata oluştu.', 'error');
    }
}
...
async function initializeNobetTakip() {
    ...
    copyBtn.removeEventListener('click', copyNobetSchedule);
    copyBtn.addEventListener('click', copyNobetSchedule);
    yazdirBtn.addEventListener('click', () => window.print());
    nobetEkleBtn.removeEventListener('click', addNobet);
    nobetEkleBtn.addEventListener('click', addNobet);
    topluSilBtn.removeEventListener('click', deleteSelectedNobetler);
...
async function initializeIzinTakip() {
    ...
    staffListForLeave.forEach(p => {
        // Eğer işten ayrılış tarihi varsa listeye ekleme
        if (p.isten_ayrilis && p.isten_ayrilis.trim() !== "") {
            return;
        }
        personelSelect.add(new Option(p.ad_soyad, p.id));
    });
    izinTipiSelect.innerHTML = '<option value="">İzin Tipi Seçiniz...</option>';
    izinTipiSnap.forEach(doc => izinTipiSelect.add(new Option(doc.data().name, doc.id)));
 ...
 showToast('İzin başarıyla kaydedildi.');
        ...
        // Otomatik görevlendirme yönlendirmesi iptal edildi.
    ...
    const topluIzinAcBtn = document.getElementById('open-toplu-izin-modal-btn');
    if (topluIzinAcBtn) {
    ...
    try {
        ...
        if (personelSnap.empty) {
            ...
            personelSnap.forEach(doc => {
                ...
                // KONTROL: Eğer işten ayrılış tarihi varsa listeye ekleme
                if (p.isten_ayrilis && p.isten_ayrilis.trim() !== "") {
                    return;
                }
